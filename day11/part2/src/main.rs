...
impl Graph {
    ...
    fn denombre(&self, src: String, targ: String) -> usize {
        let target = *self.map.get(&targ).unwrap();
        let source = *self.map.get(&src).unwrap();

        let out = self.adj.len();

        let mut d = vec![0; out + 1];
        d[source as usize] = 1;
...
fn main() {
    ...
    let a = graph.denombre("svr".into(), "fft".into())
        * graph.denombre("fft".into(), "dac".into())
        * graph.denombre("dac".into(), "out".into());

    let b = graph.denombre("svr".into(), "dac".into())
        * graph.denombre("dac".into(), "fft".into())
        * graph.denombre("fft".into(), "out".into());
